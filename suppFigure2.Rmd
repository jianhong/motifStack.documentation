...
fn = local({
  ...
  function(x) {
    i <<- i + 1
    paste('Supplemental Figure 2', LETTERS[i], ': ', x, sep = '')
  }
})
```
...
## Supplemental Figure 2A-D. basic linear and radial trees
...
```{r stackLayout, fig.width=6, fig.height=6, dpi=72, fig.cap=fn("stack layout of motifStack")}
...
```{r treeLayout, fig.width=6, fig.height=6, dpi=72, fig.cap=fn("tree layout of motifStack")}
...
```{r phylogLayout, fig.width=6, fig.height=6, dpi=72, fig.cap=fn("phylog layout of motifStack")}
##try different style of sequence logo
motifStack(pfms[sample(1:length(pfms), 20)], layout="phylog", clabel.leaves=.8, f.logo=0.6, ic.scale=FALSE)
```

### Supplemental Figure 2D
If you want to show even more motifs in one figure, you would want to try radialPhylog layout.

```{r radialPhylogLayout, fig.width=10, fig.height=10, dpi=72, fig.cap=fn("radialPhylog layout of motifStack")}
...
## Supplemental Figure 2E-G. merge motifs by different distance
...
```{r merging, fig.width=6, fig.height=6, dpi=72, fig.cap=fn("merging by different distance")}
outpath <- "output"
...
```{r motifCloud, warning=FALSE, fig.width=6, fig.height=6, dpi=72, fig.cap=fn('motifCloud')}
## get signature
gpDist <- c(.5, 1)
...
```{r motifCloudRect, warning=FALSE, fig.width=6, fig.height=6, dpi=72, fig.cap=fn('motifCloud')}
...
## Supplemental Figure 2H-M. color sets
...
```{r colorsets1, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("color setting, tree background")}
...
```{r colorsets2, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("color setting, leaves")}
plotMotifStackWithRadialPhylog(phylog=phylog, pfms=unaligned.pfms,
                            labels.leaves=leaveNames, 
...
```{r colorsets3, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("color setting, leave background")}
...
```{r colorsets4, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("color setting, inner circle")}
plotMotifStackWithRadialPhylog(phylog=phylog, pfms=unaligned.pfms,
                            labels.leaves=leaveNames, 
...
```{r colorsets5, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("color setting, outer circle")}
plotMotifStackWithRadialPhylog(phylog=phylog, pfms=unaligned.pfms,
                            labels.leaves=leaveNames, 
...
### Supplemental Figure 2M
```{r colorsets6, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("color setting, combination")}
...
## Supplemental Figure 2N-O. color sets applied to different alignment methods
...
```{r MotIV, fig.width=6, fig.height=6, dpi=72, fig.cap=fn("MotIV with different parameter")}
...
### Supplemental Figure 2O
```{r MatAlign, fig.width=12, fig.height=12, dpi=72, fig.cap=fn("UPGMA tree by matAlign+PHYLIP")}
##read newick tree. Alignment is done by MatAlign (see http://stormo.wustl.edu/MatAlign/) and the newick tree is generated by Neighbor, which is a part of phylip (see http://evolution.genetics.washington.edu/phylip/progs.data.dist.html)
outpath <- "output"
matalign_path <- "./app/matalign-v4a"
neighbor_path <- "./app/neighbor.app/Contents/MacOS/neighbor"
system(paste("perl MatAlign2tree.pl --in . --pcmpath", pcmpath, 
...
## Supplemental Figure 2P. different distance resolution with different alignment methods

```{r revolution, fig.width=12, fig.height=12, dpi=72}
pcmpath <- "pcmsDatasetDM"
pcms <- readPCM(pcmpath)
pfms<-lapply(pcms,pcm2pfm)
