...
fn = local({
  ...
  function(x) {
    i <<- i + 1
    paste('__Supplementary Figure 2', LETTERS[i], '__: ', x, sep = '')
  }
})
```
...
```{r merging, fig.width=4, fig.height=4, fig.cap=fn("merging by different distance")}
outpath <- "output"
...
```{r motifCloud, warning=FALSE, fig.width=4, fig.height=4, dpi=72, fig.cap=fn('motifCloud')}
## get signature
gpDist <- c(.5, 1)
...
```{r motifCloudRect, warning=FALSE, fig.width=4, fig.height=4, fig.cap=fn('motifCloud')}
...
```{r MotIV, fig.width=4, fig.height=4, fig.cap=fn("MotIV with different parameter")}
